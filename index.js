...
const readline = __importStar(require("readline"));
{
    ...
    console.log("3rd Assignment\n");
    {
        ...
        var num4 = 40;
        var num5 = 50;
        var sumation = num1 + num2 + num3 + num4 + num5;
        var tatalNumbers = 5;
        var percentage = (sumation / tatalNumbers);
    ...
    // // ===========================================================================================
    {
        ...
        var ProductPrice = 80; // in dollars and can vary
        var ProductPrice2 = 120; // in dollars and can vary
        // Calculating 5% discount for the product that is less than $100.
        if (ProductPrice <= 100) {
            var newProductPrice = (ProductPrice * 5) / 100; // calculating 5% discount for the product.
            var dicsountedPrice = ProductPrice - newProductPrice;
        ...
        if (ProductPrice2 > 100) {
            var newProductPrice = (ProductPrice2 * 10) / 100; // calculating 10% discount for the product.
            var dicsountedPrice = ProductPrice2 - newProductPrice;
    ...
    // // ===========================================================================================
    {
        ...
        rl.question("Enter number of units used: ", (userInput) => {
            var unitAmount = userInput;
            rl.close();
            var initialPricePerUnit = 2; // Letting {initial price per unit without any tax} In dolllars
            if (unitAmount <= 100) { // 0 tax here, as per guideline in the question.
                var Bill = (unitAmount * initialPricePerUnit);
                var calculatingTax = Bill; // Calculating 10% additional tax for (100 - 199) units.
                var actualBill = Bill;
                ...
                var Bill = (unitAmount * initialPricePerUnit);
                var calculatingTax = Bill * 10 / 100; // Calculating 10% additional tax for (100 - 199) units.
                var actualBill = Bill + calculatingTax;
                console.log(`Total Units used are ${unitAmount} units\nInitial Cost per Unit is: $${initialPricePerUnit}\nYour initial bill is: $${Bill}\nTotal tax of 10% is added that is: $${calculatingTax}\nYour total bill is: $${actualBill}`);
            }
            else if (unitAmount > 200 && unitAmount < 500) {
                var Bill = (unitAmount * initialPricePerUnit);
                var calculatingTax = Bill * 15 / 100; // Calculating 15% additional tax for (200 - 499) units.
                var actualBill = Bill + calculatingTax;
                console.log(`Total Units used are ${unitAmount} units\nInitial Cost per Unit is: $${initialPricePerUnit}\nYour initial bill is: $${Bill}\nTotal tax of 15% is added that is: $${calculatingTax}\nYour total bill is: $${actualBill}`);
            }
            else if (unitAmount > 500) {
                var Bill = (unitAmount * initialPricePerUnit);
                var calculatingTax = Bill * 25 / 100; // Calculating 25% additional tax for (greater than 500) units.
                var actualBill = Bill + calculatingTax;
    ...
    // // ===========================================================================================
    {
        console.log("write a rogram that tells if the student is pass or fail based on the attendance greater than 75%, practical marks should be greater than 50% and theory marks should be greater than 45% for passing in a semester.");
